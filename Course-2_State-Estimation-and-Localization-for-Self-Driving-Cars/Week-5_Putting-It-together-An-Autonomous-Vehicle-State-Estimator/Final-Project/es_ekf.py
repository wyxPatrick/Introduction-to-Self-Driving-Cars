...
#     t: Timestamps in ms.
################################################################################################
gt = data['gt']                 # Ground Truth IMU - inertial frame
imu_f = data['imu_f']           # Acceleration x,y,z - vehicle frame
imu_w = data['imu_w']           # Rotation - vehicle frame
gnss = data['gnss']             # GNSS
lidar = data['lidar']           # Lidar object positions - vehicle frame later
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    K = p_cov_check.dot(h_jac.T).dot(np.linalg.inv(h_jac.dot(p_cov_check).dot(h_jac.T) + sensor_var * np.eye(3)))
    # 3.2 Compute error state
    delta_x_hat = K.dot(y_k - p_check)    # GNSS and LIDAR can only give back position info!
    # 3.3 Correct predicted state
    delta_p = delta_x_hat[:3]
    delta_v = delta_x_hat[3:6]
    delta_angle = delta_x_hat[6:]

    p_hat = p_check + delta_p
    v_hat = v_check + delta_v
    q_hat = Quaternion(euler = delta_angle).quat_mult_right(q_check)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    f = imu_f.data[k-1]
    w = imu_w.data[k-1]
    p_check = p_est[k-1] + v_est[k-1] * delta_t + 0.5 * (C_ns.dot(f)+g) * delta_t**2
    v_check = v_est[k-1] + delta_t * (C_ns.dot(f)+g)
    ...
    # F_k[3:6, 6:9] = -1 * delta_t * skew_symmetric(C_ns.dot(f))
    F_k[3:6, 6:] = -(C_ns.dot(skew_symmetric(imu_f.data[k-1].reshape((3,1)))))

    # 2. Propagate uncertainty
    p_cov_check = F_k.dot(p_cov_check).dot(F_k.T) + l_jac.dot(Q_k).dot(l_jac.T)
